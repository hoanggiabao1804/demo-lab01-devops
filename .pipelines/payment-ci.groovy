def call(Map params) {
    ...
    stage('Run Maven Checkstyle') {
        ...
        -pl order \
        -am \
        -Dcheckstyle.output.file=order-checkstyle-result.xml
        '''
    }
    ...
    stage('Gitleak Scan') {
        ...
        --source ./order \
        --no-git \
        --report-path order-gitleaks-report.json \
        --report-format json \
        --exit-code 0
        '''
        ...
        '  order-gitleaks-report.json > order-gitleaks-report.html
        '''

        publishHTML([
            reportDir: '.',
            reportFiles: 'order-gitleaks-report.html',
        ...
        def hasLeak = sh(
            script: '[ grep -q "RuleID" order-gitleaks-report.json ]',
            returnStatus: true
        )
    ...
    stage('Snyk Scan') {
        ...
        snyk test --file=pom.xml --package-manager=maven -d --json > order-snyk-report.json || true
        '''
        ...
        ' order-snyk-report.json > order-snyk-report.html
        '''

        publishHTML([
            reportDir: '.',
            reportFiles: 'order-snyk-report.html',
        ...
        def hasVuln = sh(
            script: 'grep -q "vulnerabilities" order-snyk-report.json',
            returnStatus: true
        )
