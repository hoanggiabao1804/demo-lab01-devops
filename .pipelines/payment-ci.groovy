def call(Map params) {
    ...
    stage('Publish Coverage Report') {
        publishHTML([
            ...
            reportName: 'Payment JaCoCo Coverage',
            keepAll: true,
    ...
    stage('Snyk Scan') {
        def jsonReport = 'reports/snyk/payment-snyk-report.json'
        def htmlReport = 'reports/snyk/payment-snyk-report.html'
        def snykExitCode = 2
        ...
        withCredentials([
            ...
            snykExitCode = sh(
                ...
                script: '''
                    ...
                    snyk test \
                        --file=payment/pom.xml \
                        --maven-skip-wrapper \
                        --org="$SNYK_ORG" \
                        --json-file-output=reports/snyk/payment-snyk-report.json \
                        -- \
                        -Drevision="$REVISION"
                '''
            )
    ...
    stage('Build and Push Docker Image') {
        ...
        docker build -t 23120022/yas-payment:$IMAGE_TAG ./payment

        docker push 23120022/yas-payment:$IMAGE_TAG
        '''
    }
}
