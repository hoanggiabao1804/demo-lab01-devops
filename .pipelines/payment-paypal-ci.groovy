def call(Map params) {
    ...
    stage('Publish Coverage Report') {
        publishHTML([
            ...
            reportName: 'Payment-Paypal JaCoCo Coverage',
            keepAll: true,
    ...
    stage('Snyk Scan') {
        def jsonReport = 'reports/snyk/payment-paypal-snyk-report.json'
        def htmlReport = 'reports/snyk/payment-paypal-snyk-report.html'
        def snykExitCode = 2
        ...
        withCredentials([
            ...
            snykExitCode = sh(
                ...
                script: '''
                    ...
                    snyk test \
                        --file=payment-paypal/pom.xml \
                        --maven-skip-wrapper \
                        --org="$SNYK_ORG" \
                        --json-file-output=reports/snyk/payment-paypal-snyk-report.json \
                        -- \
                        -Drevision="$REVISION"
                '''
            )
