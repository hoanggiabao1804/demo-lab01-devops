def call(Map params) {
    ...
    stage('Run Maven Checkstyle') {
        ...
        -pl payment-paypal \
        -am \
        -Dcheckstyle.output.file=payment-paypal-checkstyle-result.xml
        '''
    }
    ...
    stage('Gitleak Scan') {
        ...
        --no-git \
        --report-path payment-paypal-gitleaks-report.json \
        --report-format json \
        --exit-code 0
        '''
        ...
        '  payment-paypal-gitleaks-report.json > payment-paypal-gitleaks-report.html
        '''

        publishHTML([
            reportDir: '.',
            reportFiles: 'payment-paypal-gitleaks-report.html',
        ...
        def hasLeak = sh(
            script: '[ grep -q "RuleID" payment-paypal-gitleaks-report.json ]',
            returnStatus: true
        )
    ...
    stage('Snyk Scan') {
        ...
        snyk test --file=pom.xml --package-manager=maven -d --json > payment-paypal-snyk-report.json || true
        '''
        ...
        ' payment-paypal-snyk-report.json > payment-paypal-snyk-report.html
        '''

        publishHTML([
            reportDir: '.',
            reportFiles: 'payment-paypal-snyk-report.html',
        ...
        def hasVuln = sh(
            script: 'grep -q "vulnerabilities" payment-paypal-snyk-report.json',
            returnStatus: true
        )
