def call(Map params) {
    ...
    stage('Publish Coverage Report') {
        publishHTML([
            ...
            reportName: 'Customer JaCoCo Coverage',
            keepAll: true,
    ...
    stage('Snyk Scan') {
        def jsonReport = 'reports/snyk/customer-snyk-report.json'
        def htmlReport = 'reports/snyk/customer-snyk-report.html'
        def snykExitCode = 2
            ...
            snykExitCode = sh(
                ...
                script: '''
                    ...
                    snyk test \
                        ...
                        --maven-skip-wrapper \
                        --org="$SNYK_ORG" \
                        --json-file-output=reports/snyk/customer-snyk-report.json \
                        -- \
                        -Drevision="$REVISION"
                '''
            )
    ...
    stage('Build and Push Docker Image') {
        ...
        docker build -t 23120022/yas-customer:$IMAGE_TAG ./customer

        docker push 23120022/yas-customer:$IMAGE_TAG
        '''
    }
}
