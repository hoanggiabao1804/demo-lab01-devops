def call(Map params) {
    ...
    stage('Gitleak Scan') {
        ...
        publishHTML([
            reportDir: '.',
            reportFiles: 'reports/gitleaks/backoffice-bff-gitleaks-report.html',
    ...
    stage('Snyk Scan') {
        ...
        snyk test --file=pom.xml --package-manager=maven -d --json > reports/snyk/backoffice-bff-snyk-report.json || true
		'''

        def snykUtils = load '.pipelines/utils/snyk-utils.groovy'
        snykUtils.jsonToHtml(
            'reports/snyk/backoffice-bff-snyk-report.json', 
            'reports/snyk/backoffice-bff-snyk-report.html'
        )

		publishHTML([
			reportDir: '.',
			reportFiles: 'reports/snyk/backoffice-bff-snyk-report.html',
