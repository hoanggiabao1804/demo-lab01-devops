def call(Map params) {
    ...
    stage('Snyk Scan') {
        def jsonReport = 'reports/snyk/sampledata-snyk-report.json'
        def htmlReport = 'reports/snyk/sampledata-snyk-report.html'
        def snykExitCode = 2
        ...
        withCredentials([
            ...
            snykExitCode = sh(
                ...
                script: '''
                    ...
                    snyk test \
                        --file=sampledata/pom.xml \
                        --maven-skip-wrapper \
                        --org="$SNYK_ORG" \
                        --json-file-output=reports/snyk/sampledata-snyk-report.json \
                        -- \
                        -Drevision="$REVISION"
                '''
            )
    ...
    stage('Build and Push Docker Image') {
        ...
        docker build -t 23120022/yas-sampledata:$IMAGE_TAG ./sampledata

        docker push 23120022/yas-sampledata:$IMAGE_TAG
        '''
    }
}
