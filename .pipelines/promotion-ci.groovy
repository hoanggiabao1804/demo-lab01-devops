def call(Map params) {
    ...
    stage('Snyk Scan') {
        def jsonReport = 'reports/snyk/promotion-snyk-report.json'
        def htmlReport = 'reports/snyk/promotion-snyk-report.html'
        def snykExitCode = 2
        ...
        withCredentials([
            ...
            snykExitCode = sh(
                ...
                script: '''
                    ...
                    snyk test \
                        --file=promotion/pom.xml \
                        --maven-skip-wrapper \
                        --org="$SNYK_ORG" \
                        --json-file-output=reports/snyk/promotion-snyk-report.json \
                        -- \
                        -Drevision="$REVISION"
                '''
            )
